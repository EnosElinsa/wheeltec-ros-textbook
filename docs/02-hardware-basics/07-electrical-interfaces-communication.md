...
串行通信（Serial Communication）按顺序逐位传送数据。串行端口（Serial Port）是一对设备之间交换连续字节的接口。Ubuntu 通常把 USB 转串口设备表示为 `/dev/ttyUSB0` 或 `/dev/ttyACM0`。端口编号会随插拔顺序改变，稳定的设备别名需要 udev 规则、序列号或固定物理端口共同保证。
