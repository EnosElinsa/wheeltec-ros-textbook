...
本章通过外观、标签和接线关系核对供电系统。控制板保持封闭，带电电路保持原状；涉及万用表、裸露端子或电池直连的检查，应由熟悉低压直流系统的人完成。
...
电池是能量源。电池输出可能直接送给电机驱动器，也可能经过变压模块后再给主控、传感器或 USB 集线器供电。接入设备前核对电压范围、持续电流能力、极性和接头；外形相同的插头也可能对应不同电压或极性。

供电范围随产品系列、电池化学体系和硬件版本变化。教育机器人常见约 10–12.6 V，大型科研机器人常见约 20–25.2 V；部分 R680 批次要求电池高于 20 V 才进入控制。这些范围只用于识别供电等级，实际操作以电池和设备标签为准。

### 信号链路

ROS 主控通常通过 USB 转串口与底盘控制板通信。串行端口（Serial Port）把字节按顺序传送，主控上的 USB 转串口线缆把 USB 信号转换成控制板能接收的串行信号。雷达、相机和部分惯性测量单元（Inertial Measurement Unit, IMU）也通过 USB 传输数据；USB 在很多设备上同时承担 5 V 供电。接口不足时可以使用带独立供电的 USB 集线器，但集线器供电能力、接地和线缆质量会影响设备稳定性。

底盘内部的控制信号从 STM32 控制板进入电机驱动器，再由驱动器输出足够的电压和电流驱动电机。编码器信号反向回到控制板，用于测速和闭环控制。控制板与扩展模块之间还可能使用控制器局域网（Controller Area Network, CAN）；它用 CAN_H 和 CAN_L 两根信号线让多个设备共享同一条总线。
...
按第 10 章识别出的底盘类型，打开对应接线资料。R680 的阿克曼、差速/四轮两驱、麦轮/四驱、全向轮分别有接线说明，电机编号和转向结构并不相同。
...
资料核对可以证明线路符合文档，但不能代替带载实机验证。本章状态为“资料核对通过，实机未验证”，直到第 12 章完成首次上电基线。
...
[上一章：识别你的机器人配置](10-identify-configuration.md) · [返回本篇](index.md) · [下一章：首次上电与基线检查](12-first-power-on.md)
