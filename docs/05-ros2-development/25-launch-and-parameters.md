...
启动链中还可能出现EKF和URDF。EKF 用多源测量估计机器人状态；URDF 用连杆和关节描述车型模型。看到这些名称时，先把它们当作启动链中的独立节点或资源，再继续核对参数和输出。
...
启动链闭合不等于数据链闭合：某个雷达或相机进程被启动，只能证明 Launch 创建了它；还要检查EKF配置、重映射（remap）、话题类型、频率和 TF，才能证明数据真正被消费。
...
[上一章：节点、话题、服务与动作](24-nodes-topics-services-actions.md) · [返回本篇](index.md) · [下一章：TF、URDF 与 RViz](26-tf-urdf-rviz.md)
