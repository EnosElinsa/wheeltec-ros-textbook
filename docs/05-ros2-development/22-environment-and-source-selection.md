...
机器人模型常用结构化描述文件保存连杆、关节和几何信息。第 25 章会在启动链中追踪它的加载位置，第 26 章再说明模型、坐标变换与显示工具的关系。

## 22.5 源码包确认 {#source-package-selection}

这份导读用于回答一个具体问题：执行一条启动命令后，ROS 2 怎样从启动文件走到底盘串口节点、参数、话题、传感器和 TF？读者应在自己的机器人工作空间中完成追踪，不要把本页出现的文件名当成所有版本都相同的固定清单。
...
| `CMakeLists.txt` 或 `setup.py` | CMake/ament 或 Python 安装规则 | 实际生成哪些可执行文件；启动、配置和资源是否被安装 |
| `launch/`（若有） | 组合底盘、状态估计、IMU、模型、雷达和相机 | 哪个是入口；它继续包含了哪些启动文件 |
| `config/`（若有） | 状态估计、IMU、相机等参数 | 参数文件是否真的被入口引用 |
