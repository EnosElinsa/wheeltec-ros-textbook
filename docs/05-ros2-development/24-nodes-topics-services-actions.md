...
# 24. 节点、话题、服务与动作 {#pubsub-code-resource}
...
接口类型规定消息字段。名字相同但类型不同的接口不能直接通信。ROS 2 还受域标识符（Domain Identifier, Domain ID）、数据分发服务（Data Distribution Service, DDS）实现和服务质量（Quality of Service, QoS）策略影响。
...
先完成第 23 章的构建步骤。终端 A 运行 [C++ 示例](https://github.com/EnosElinsa/wheeltec-ros-source-reference/tree/62aacaed782091b06060ace276c64e9aaf84ae6b/examples/ros2/pubsub/cpp)发布者；查看 `package.xml`、`CMakeLists.txt`、`src/publisher.cpp` 和 `src/subscriber.cpp`：
...
终端 B 运行 [Python 示例](https://github.com/EnosElinsa/wheeltec-ros-source-reference/tree/62aacaed782091b06060ace276c64e9aaf84ae6b/examples/ros2/pubsub/python)订阅者；查看 `package.xml`、`setup.py`、`py_pubsub/publisher.py` 和 `py_pubsub/subscriber.py`：
...
确认类型为 `std_msgs/msg/String`，并至少有一个发布者和一个订阅者。停止两个节点后交换语言再运行一次；C++ 发布、Python 订阅以及相反组合都能收到消息时，接口才算跨语言兼容。
...
- 能指出服务与动作相对话题的区别。
- 多机通信时记录 Domain ID 和 QoS；`ping` 只能说明网络层可达，不能证明 ROS 2 接口匹配。

## 24.7 故障排查

涉及传感器时，先确认设备能被系统识别，再检查驱动节点；随后核对话题类型和 QoS。数据可见但上层仍失败时，检查时间戳与坐标关系，最后再查看动作服务器或其他算法节点。
