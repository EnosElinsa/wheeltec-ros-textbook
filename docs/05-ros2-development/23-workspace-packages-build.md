...
## 23.4 工作原理 {#workspace-layout}

### 工作空间的目录关系

工作空间（Workspace）是一个为机器人程序准备的项目目录。`src/` 存源码，`build/` 存构建中间文件，`install/` 存可加载结果，`log/` 存构建日志。功能包（Package）是工作空间中的可构建单元，例如底盘驱动或相机驱动。终端执行 `source install/setup.bash` 后，ROS 才会优先看到这次构建的包；下一章将用这些包提供的节点和话题检查通信。
...
`--symlink-install` 会为部分 Python、启动文件和配置资源建立符号链接，便于迭代，但 C/C++ 修改仍需重新构建。不能据此断言所有配置修改都永远无需构建；包的安装规则决定实际行为。
...
代码仓库提供两个独立功能包：[C++ 示例](https://github.com/EnosElinsa/wheeltec-ros-source-reference/tree/ebae2342709c756cb8fa387ccdbd6e5733f9219a/examples/ros2/pubsub/cpp)和 [Python 示例](https://github.com/EnosElinsa/wheeltec-ros-source-reference/tree/ebae2342709c756cb8fa387ccdbd6e5733f9219a/examples/ros2/pubsub/python)。把它们放进同一工作空间的 `src/` 后构建：
...
git -C wheeltec-ros-source-reference checkout ebae2342709c756cb8fa387ccdbd6e5733f9219a
...
构建失败先检查网络、设备依赖和驱动包，再确认节点是否真的从当前工作空间加载。构建通过却不能通信时，继续按话题、时间/坐标、算法的顺序检查；不要把运行时现象重新归为编译错误。
...
[上一章：ROS 2 环境与源码版本选择](22-environment-and-source-selection.md) · [返回本篇](index.md) · [下一章：节点、话题、服务与动作](24-nodes-topics-services-actions.md)
