...
ROS 2 工作空间（ROS 2 Workspace）是一个为机器人程序准备的项目目录。`src/` 存源码，`build/` 存构建中间文件，`install/` 存可加载结果，`log/` 存构建日志。功能包是工作空间中的可构建单元，例如底盘驱动或相机驱动。终端执行 `source install/setup.bash` 后，ROS 才会优先看到这次构建的包；下一章将用这些包提供的节点和话题检查通信。
