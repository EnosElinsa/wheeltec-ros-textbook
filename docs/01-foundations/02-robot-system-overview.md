...
上一章把 ROS 2 放在机器人软件层中，本章沿着一次避障过程观察整台机器。传感器提供环境信息，主控运行 ROS 2 程序，控制板执行速度目标，电机与机械结构产生运动。供电系统则为这条工作链提供能量。
...
**传感器**把环境或机器人自身状态转换成程序可读的数据。常见设备包括：
...
**主控**是机器人上的计算机，常见平台包括树莓派、Jetson、RDK、Atlas 和工控机。它运行 Ubuntu、ROS 2 驱动以及定位、导航、视觉等程序，并根据传感器数据产生运动目标。

主控的优势在于计算能力和软件扩展性。更换算法、记录日志或增加摄像头处理，通常都在这一层完成。它输出的是速度、位置或任务目标，并不直接向大功率电机供电。

## 2.4 控制：控制板怎样执行速度目标

**控制板**靠近底盘硬件，通常采用 STM32 系列微控制器。它按固定周期读取编码器、惯性传感器和其他底层信号，完成**底层反馈控制**，再把计算结果交给电机驱动器。

主控与控制板常通过串行通信或**控制器局域网（Controller Area Network, CAN）**交换数据。主控发送期望速度，控制板返回车轮速度、里程和设备状态。两端采用相同的波特率、数据格式和校验规则，信息才能被正确解析。

## 2.5 执行：电机与机械结构怎样产生运动

**电机驱动器**根据控制信号调节流向电机的电流，电机再把电能转换成转动。减速机构降低输出转速、提高力矩，车轮和车架最终把转动变成整车运动。
