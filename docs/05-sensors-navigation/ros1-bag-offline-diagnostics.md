...
把 `BAG_SOURCE` 改成实际绝对路径；引号保留，以处理空格。严格模式会让路径、哈希或复制失败立即停止；两个哈希非空且 `cmp` 退出码为 0 才继续。后续操作只针对 `BAG_COPY`。若 `rosbag info` 失败，不要对归档原件执行 `reindex` 或 `fix`；只在副本上诊断。结束前把 `bag-info.yaml`、两个哈希文件和实验记录移到长期保存位置，临时目录才可以删除。
