...
激光雷达（Light Detection and Ranging, LiDAR）按角度连续测量距离，发布的二维扫描让机器人能看到墙面和障碍物。同步定位与建图（Simultaneous Localization and Mapping, SLAM）把这些扫描与机器人运动估计配准，一边估计自己在哪，一边逐步生成地图。它依赖雷达数据、里程计和 TF 坐标关系；后续第 27 章会使用这张地图做定位，第 28 章的 Nav2 再根据定位结果规划路径。
...
先检查雷达供电、连接和安装方向，再确认驱动节点及扫描话题。扫描稳定后核对时间戳、里程计和 TF；这些数据正确时，才调整 SLAM 参数或保存流程。
