...
![三份运动输入进入 EKF 融合。轮速、陀螺仪和视觉分别提供速度量，EKF 按方差调整它们的权重并输出 `/odom`](assets/sensor-fusion.png?v=3)
...
![导航失败后的恢复顺序。先清除对应代价地图重试，仍失败时轮流清图、倒退或等待，再重新规划](assets/recovery-sequence.png?v=3)
