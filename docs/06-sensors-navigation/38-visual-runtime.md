...
![三份运动输入进入 EKF 融合。轮速、陀螺仪和视觉分别提供速度量，EKF 按方差调整它们的权重并输出 `/odom`](assets/sensor-fusion.png)
...
![MPPI 控制器的计算过程。由当前状态生成多组未来轨迹，按代价确定权重，再加权平均并只执行第一步速度](assets/mppi-control.png)
...
![导航失败后的恢复顺序。先清除对应代价地图重试，仍失败时轮流清图、倒退或等待，再重新规划](assets/recovery-sequence.png)
...
![看门狗的速度转发和停车条件。正常时转发 `/cmd_vel`，检测到速度超时或视觉、轮速、定位异常时发送全零速度](assets/velocity-watchdog.png)
