---
status: complete
---

# 33. 多点导航、路径跟踪与自主探索 {#path-follow-code-resource}

## 33.1 学习目标

在单目标 Nav2 已通过后，增加任务层功能：按序访问多个目标、跟踪给定路径或自动选择探索目标。失败时仍回到 Nav2 基础链诊断。

!!! example "本章代码：路径跟踪"
    仓库目录：[`ros2/navigation/path-follow/wheeltec_path_follow`](https://github.com/EnosElinsa/wheeltec-ros-source-reference/tree/ebae2342709c756cb8fa387ccdbd6e5733f9219a/ros2/navigation/path-follow/wheeltec_path_follow)

    完整文件集见 `metadata/code-resource-manifests/path-follow.txt`；本节重点查看 `package.xml`、`launch/follow_path.launch.py`、`scripts/follow_path.py`。验收：能定位路径跟踪入口。边界：不替代实机安全检查。

## 33.2 适用范围

ROS 2 多点导航、路径跟踪和快速扩展随机树（Rapidly-Exploring Random Tree, RRT）或前沿探索类功能。RRT 在可通行空间中随机扩展树结构以寻找路径；前沿探索则把已知区域与未知区域的边界作为候选目标。具体包和消息随软件版本变化。

## 33.3 操作前检查

- 第 32 章单目标导航稳定。
- 取消动作和急停已经验证。
- 地图边界与禁行区明确。

## 33.4 工作原理 {#exploration-methods}

多点导航管理一组目标及失败策略；路径跟踪给控制器一条由多个位置点组成的离散路径；自主探索根据未知区域生成新目标。三类任务都复用已经运行的定位、代价地图和控制器，不再启动另一套底盘驱动或 TF。

## 33.5 操作步骤

1. 多点导航先使用两个近距离目标，记录到达顺序、等待和失败策略。
2. 路径跟踪先检查路径坐标系、点间距和方向，再低速运行。
3. 自主探索设置地图边界、最小目标距离和停止条件，清除危险区域。
4. 运行时观察当前目标、动作反馈、全局路径和 `cmd_vel`。
5. 主动取消一次，确认任务层和底盘都停止。

## 33.6 验收标准

- 目标/路径坐标系与地图一致。
- 单个目标失败不会让任务无限重试或越界。
- 取消后不再产生非零速度。
- 探索结束条件明确，地图可保存。

## 33.7 故障排查

| 现象 | 处理 |
|---|---|
| 多点顺序混乱 | 查列表生成、坐标系和索引 |
| 路径偏离地图 | 查 frame_id、原点和路径转换 |
| 探索反复选同一点 | 查黑名单、前沿阈值和定位稳定性 |
| 取消后继续走 | 查动作取消、任务线程和速度发布者 |

## 章节导航

[上一章：Nav2 导航](32-nav2-navigation.md) · [返回本篇](index.md) · [下一章：摄像头与图像话题](34-camera-opencv-images.md)
