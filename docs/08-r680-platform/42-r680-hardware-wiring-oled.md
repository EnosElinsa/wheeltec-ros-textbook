...
识别 R680 的底盘形式、C50 系列控制板、编码器和主要线束，并能从**有机发光二极管（Organic Light-Emitting Diode, OLED）显示屏**读取电压、车型、自检、控制模式及电机目标/反馈。完成本章后，读者可以根据实机丝印、接口和记录选择对应接线图。

## 42.2 适用范围

R680 可能使用两类转速反馈器件。**巨磁阻（Giant Magnetoresistance, GMR）编码器**利用磁阻随磁场变化的特性检测转动；**霍尔效应编码器（Hall Effect Encoder）**利用霍尔元件检测磁场变化。两者都能输出转动脉冲，但供电、接线、脉冲特性和配套固件可能不同。
